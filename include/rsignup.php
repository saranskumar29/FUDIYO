...
if (empty($orgname)) {
    $em = "Organisation name is required";
    header("Location: ../rlogin.php?error=$em&$data");
    exit;
}else if(empty($orgid)){
    $em = "Organisation Id is required";
    header("Location: ../rlogin.php?error=$em&$data");
    exit;
    ...
    header("Location: ../rsignup.php?error=$em&$data");
    exit;
}else {
    ...
    $uploadSuccess = move_uploaded_file($_FILES["fileToUpload"]["tmp_name"],$targetFile);
    	...
    	$sql = "INSERT INTO rsignup(orgname, orgid, email, phone, password, confirmpassword,filepath,verify_status) 
    	        VALUES(?,?,?,?,?,?,?,?)";
    	$stmt = $conn->prepare($sql);
    	$stmt->execute([$orgname, $orgid, $email, $phone, $pass, $confpass,$targetFileName,1]);

    	header("Location: ../rsignup.php?success=Your account has been created successfully");
	    exit;
    }
